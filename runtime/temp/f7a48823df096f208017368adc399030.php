<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:58:"F:\www\tp\public/../application/home\view\server\live.html";i:1512029005;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="utf-8">
        ...
        <div class="row noticeList">
            <a href="<?php echo url('Article/detail?id='.$inform['id']); ?>">
                ...
                <div class="col-xs-10">
                    ...
                    <p class="intro">所属分类:<span><?php echo get_category_title($inform['category_id']); ?></span></p>
                </div>
            </a>
        </div>
        <?php endforeach; endif; else: echo "" ;endif; ?>
...
<script type="text/javascript">
    ...
    $("#nextpage").click(function () {
        page++
        $.getJSON("/home/server/ajaxLive",{"page":page},function (data) {
            var data = $.parseJSON(data).data
            if (data.length != 0){
                $.each(data,function () {
                    var that = this;
                    //>>发送Ajax请求获取小区通知的封面图片路径和更新时间
                    $.post("/home/inform/message",{"cover_id":this.cover_id,"category_id":this.category_id,"update_time":this.update_time},function (data) {
                        var cover = $.parseJSON(data).cover;
                        var time = $.parseJSON(data).time;
                        var category_name = $.parseJSON(data).category_name;
                        ...
                        str += '<p class="intro">所属分类:<span>'+category_name+'</span></p>';
                        str += '</p> </div> </a> </div>';
                        $("#inform").append(str);
                    })
                })
